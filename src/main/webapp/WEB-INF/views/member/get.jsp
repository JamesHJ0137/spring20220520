...
<script>
	$(document).ready(function() {
		...
		// 이메일 input 요소에 text 변경시 이메일 중복 확인 버튼 활성화
		...
		// 닉네임 input 요소에 text 변경시 닉네임 중복 확인 버튼 활성화
			...
			$.ajax({
				...
				success : function(data) {
					...
					case "ok" :
						...
						break;
					case "nope" :
						$("#emailMessage1").text("이미 사용중인 이메일이거나 사용할 수 없는 닉네임입니다.");
						break;
					}
				},
				error : function() {
					$("#emailMessage1").text("이메일 중복 확인 중 문제 발생 다시 시도해 주세요.");
				},
				complete : function() {
			...
			$.ajax({
				...
				success : function(data) {
					...
					case "ok" :
						...
						break;
					case "nope" :
						$("#nickNameMessage1").text("이미 사용중인 닉네임이거나 사용할 수 없는 닉네임입니다.");
						break;
					}
				},
				error : function() {
					$("#nickNameMessage1").text("닉네임 중복 확인 중 문제 발생 다시 시도해주세요.");
				},
				complete : function() {
		...
		$("#modifySubmitButton2").click(function(e) {
			...
			// input 값 옮기기
			form2.find("[name=passwrd]").val($("#passwordInput1").val());
...
<body>
	...
	<div>
		...
		<button id="emailCheckButton1" disabled>이메일 중복 확인</button>
		<br />
		닉네임 :
		<input id="nickNameInput1" type="text" value="${member.nickName }" />
		<button disabled>닉네임 중복 확인</button>
		<br />
	...
	2. 닉네임 input에 변경 발생시 '닉네임중복확인버튼 활성화'
		-> 버튼 클릭시 ajax로 요청 응답, 적절한 메시지 출력
	3. 암호 / 암호확인일치, 이메일 중복확인 완료, 
		닉네임 중복확인 완료 시에만 수정버튼 활성화
	 --%>
